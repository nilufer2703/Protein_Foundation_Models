...
output_dir = Path(f"/dapustor/nilufer/jonathan/ProteinMPNN/Results/topk_negative_overlap_analysis/{EXPERIMENT}")
...
def get_topk_overlap(df, score_col, k):
    target_topk = df.sort_values("target", ascending=True).head(k)
    model_topk = df.sort_values(score_col, ascending=True).head(k)
...
for split_name, split_df in split_dfs.items():
    ...
    for model in models:
        ...
        for k in topk_values:
            if len(split_df) < k:
                print(
                    f"  Skipping Bottom-{k} for {split_name}/{model_name}: "
...
for split_name in summary_df["split"].unique():
    ...
    plt.ylabel("Overlap count with true Target Bottom-K")
    plt.title(f"{split_name.upper()} Bottom-K Retrieval Overlap with Target Ranking")
