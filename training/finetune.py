...
def make_batch_from_pdb(pdb_id, visible_chains=[], masked_chains=[], seq_override=None, data_path=None):
    ...
    #     visible_chains = [chain]
    # if masked_chains is None:
    #     masked_chains = []

    # if not visible_chains and not masked_chains:
    #     visible_chains = [chain]
...
def main(args):
    ...
    flip2_train_df = dpo_df[
        dpo_df["split"].astype(str).str.lower().eq("train")
    ].copy()

    flip2_val_df = dpo_df[
        dpo_df["split"].astype(str).str.lower().eq("validation")
    ].copy()
    ...
    real_pair_df = pd.read_csv(args.real_pair_csv)

    real_pair_rows = []
    for i, row in real_pair_df.iterrows():
        pos_seq = str(row["Positive_sequence"]).strip()
        neg_seq = str(row["Negative_sequence"]).strip()

        if not pos_seq or not neg_seq:
            continue

        real_pair_rows.append({
            "pair_id": str(row["Sequence_ID"]) if "Sequence_ID" in real_pair_df.columns else f"real_pair_{i}",
    ...
    logger.info(f"Real precomputed pairs loaded: {len(real_pair_rows)}")
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:
        ...
        for e in range(args.num_epochs):
            ...
            for step in progress_bar:
                ...
                num_real = min(args.real_pairs_per_step, len(real_pair_rows))
                real_pair_items = random.sample(real_pair_rows, k=num_real) if num_real > 0 else []
...
if __name__ == "__main__":  
    ...
    argparser.add_argument("--flip2_backbone_noise", type=float, default=0.5)
    ...
    argparser.add_argument("--real_pair_csv", type=str, default="/dapustor/nilufer/ProteinMPNN/flip2_data/real_negative_pairs.csv",required=True)
